...
foreach ($files as $file)
{
    ...
    { 	$TEN_DICH_VU = tag_contents($chitiet, "<TEN_DICH_VU>", "</TEN_DICH_VU>");
		if (empty($TEN_DICH_VU)) {$TEN_DICH_VU = tag_contents($chitiet, "<TEN_VAT_TU>", "</TEN_VAT_TU>");}
    ...
    foreach ($chitiets as $chitiet)
    {
		...
		$NGAY_TH_YL = tag_contents($chitiet, "<NGAY_TH_YL>", "</NGAY_TH_YL>");
        $NGAY_TH_YL = $NGAY_TH_YL[0];
        $NGAY_TH_YL = strtotime($NGAY_TH_YL);
        $NGAY_TH_YL = date('H:i d/m/Y', $NGAY_TH_YL);
		...
		$MA_MAY = tag_contents($chitiet, "<MA_MAY>", "</MA_MAY>");
		$MA_MAY = $MA_MAY[0];
		$MA_MAY = preg_replace('~[\r\n]+~', '', $MA_MAY); 
        ...
        $NGUOI_THUC_HIEN = tag_contents($chitiet, "<NGUOI_THUC_HIEN>", "</NGUOI_THUC_HIEN>");
