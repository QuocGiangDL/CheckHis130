...
foreach ($files as $file)
{
	...
	{ $TEN_DICH_VU = tag_contents($chitiet2, "<TEN_DICH_VU><![CDATA[", "]]></TEN_DICH_VU>");
		...
		$BS_CHI_DINH = str_replace('053781/HCM-CCHN', 'Bs Việt', $BS_CHI_DINH);
		$BS_CHI_DINH = str_replace('002056/ĐNA-CCHN', 'Bs Nhân', $BS_CHI_DINH);
		$BS_CHI_DINH = str_replace('0035526/HCM-CCHN', 'KTV Đài', $BS_CHI_DINH);
        }
		if (strpos($TEN_DICH_VU, 'Đo khúc xạ giác mạc') !== false)
        {
			...
			$NTH_JAVAL = tag_contents($chitiet2, "<MA_BAC_SI>", "</MA_BAC_SI>");$NTH_JAVAL = $NTH_JAVAL[0];
			...
			if (strpos($TEN_DICH_VU, 'Đo nhãn áp') !== false)
        {
			...
			$NTH_DONHANAP = tag_contents($chitiet2, "<MA_BAC_SI>", "</MA_BAC_SI>");$NTH_DONHANAP = $NTH_DONHANAP[0];
        }
		if (strpos($TEN_DICH_VU, 'Soi đáy mắt trực tiếp') !== false)
        {
			...
			$NTH_SDM = tag_contents($chitiet2, "<MA_BAC_SI>", "</MA_BAC_SI>");$NTH_SDM = $NTH_SDM[0];
        }
		if (strpos($TEN_DICH_VU, 'Siêu âm mắt') !== false)
        {
			...
			$NTH_SIEUAMMAT = tag_contents($chitiet2, "<MA_BAC_SI>", "</MA_BAC_SI>");$NTH_SIEUAMMAT = $NTH_SIEUAMMAT[0];
        }
		if (strpos($TEN_DICH_VU, 'Định lượng Glucose') !== false)
        {
			...
			$NTH_XN_MAU = tag_contents($chitiet2, "<MA_BAC_SI>", "</MA_BAC_SI>");$NTH_XN_MAU = $NTH_XN_MAU[0];
        }
		if ((strpos($TEN_DICH_VU, 'OCT') !== false) or (strpos($TEN_DICH_VU, 'Chụp đáy mắt') !== false))
        {
			...
			$NTH_OCT = tag_contents($chitiet2, "<MA_BAC_SI>", "</MA_BAC_SI>");$NTH_OCT = $NTH_OCT[0];
        }
	}
	if (strpos($TEN_CAN_LAM_SANG2, 'Đo khúc xạ giác mạc Javal') !== false)
	{$NGAY_YL = $NGAY_YL_JAVAL;$NGAY_KQ = $NGAY_KQ_JAVAL;$NGUOI_THUC_HIEN = $NTH_JAVAL;}
	elseif (strpos($TEN_CAN_LAM_SANG2, 'Đo nhãn áp') !== false)
	{$NGAY_YL = $NGAY_YL_DONHANAP;$NGAY_KQ = $NGAY_KQ_DONHANAP;$NGUOI_THUC_HIEN = $NTH_DONHANAP;}
	elseif (strpos($TEN_CAN_LAM_SANG2, 'Siêu âm mắt') !== false)
	{$NGAY_YL = $NGAY_YL_SIEUAMMAT;$NGAY_KQ = $NGAY_KQ_SIEUAMMAT;$NGUOI_THUC_HIEN = $NTH_SIEUAMMAT;}
	elseif (strpos($TEN_CAN_LAM_SANG2, 'Soi đáy mắt trực tiếp') !== false)
	{$NGAY_YL = $NGAY_YL_SDM;$NGAY_KQ = $NGAY_KQ_SDM;$NGUOI_THUC_HIEN = $NTH_SDM;}
	elseif ((strpos($TEN_CAN_LAM_SANG2, 'OCT') !== false) or (strpos($TEN_CAN_LAM_SANG2, 'Chụp đáy mắt') !== false))
	{$NGAY_YL = $NGAY_YL_OCT;$NGAY_KQ = $NGAY_KQ_OCT;$NGUOI_THUC_HIEN = $NTH_OCT;}
	else
	{$NGAY_YL = $NGAY_YL_XN_MAU;$NGAY_KQ = $NGAY_KQ_XN_MAU;$NGUOI_THUC_HIEN = $NTH_XN_MAU;}
		...
		$NGUOI_THUC_HIEN = str_replace('0035526/HCM-CCHN', 'KTV Đài', $NGUOI_THUC_HIEN);
