...
if ($_GET['id'] == 2)
{
    ...
    $typecheck = "ZIP CONVERT MẮT SÀI GÒN BẾN TRE";
}
else
{
    $files = glob("4210/4210/*.xml");
    $typecheck = "ZIP GỐC MẮT SÀI GÒN BẾN TRE";
}

echo ' <style>.gradient-custom {
    background: #f6d365;

background: linear-gradient(to bottom, #66ffff 0%, #33ccff 100%);
...
foreach ($files as $file)
{
    ...
    {
        echo "<span style='color:red'> <b>LỖI GIỜ VÀO 1 </b></span><br>";
        $cvdo = $cvdo + 1;
    }
    if ($ngayvao_limit4 < $ngayvao)
    {
        echo "<span style='color:red'> <b>LỖI GIỜ VÀO 2</b></span><br>";
        $cvdo = $cvdo + 1;
    }
    if (($ngayvao_limit2 < $ngayvao) and ($ngayvao_limit3 > $ngayvao))
    {
        echo "<span style='color:red'> <b>LỖI GIỜ VÀO</b></span><br>";
        $cvdo = $cvdo + 1;
    }
	 if ($ngayvao > $ngayra)
    {
 ...
 ';
    ...
    if ($content5)
    {
        ...
        echo '<table style="width:100%" class="table table-hover "><thead class="table-info"><tr>
			...
			<th class="col-2" scope="col">Diễn biến</th>
			<th class="col-2" scope="col" >Giờ YL</th>
			<th class="col-6" scope="col" width="50%">Phẫu thuật</th>
			</tr></thead><tbody>';
        ...
        foreach ($chitiets as $chitiet)
        {
            ...
            else
            {
                $color = 'red';
                $tdo = $tdo + 1;
                $loi = "<span style='color:red'>LỖI ZZ</span>";
            }
  ...
  <tbody>';
            ...
            foreach ($chitiets as $chitiet)
            {
                ...
                else
                {
                    $color = 'red';
                    $cvdo = $cvdo + 1;
                    $loi = " <span style='color:red'>LỖI VÀO RA </span>";
                }
