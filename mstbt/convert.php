...
foreach ($files as $file)
{
    ...
    foreach ($chitiets as $chitiet)
    {
		...
		if ((strpos($chitiet, 'Phẫu thuật cố định IOL thì hai + cắt dịch kính') !== false))
        {
            $ngayyl_pt = tag_contents($chitiet, "<NGAY_YL>", "</NGAY_YL>");
            $ngayyl_thuoc_new = strtotime($ngayyl_pt[0]) + 5400;
            $ngayyl_thuoc_new = date('YmdHi', $ngayyl_thuoc_new);
            //kq pt//
            $ngaykq_pt_iolthi2_catdichkinh = strtotime($ngayyl_pt[0]) + 1800;
            $ngaykq_pt_iolthi2_catdichkinh = date('YmdHi', $ngaykq_pt_iolthi2_catdichkinh);
            $da_lam = 1;
        }
        ...
        if ((strpos($chitiet, 'Phẫu thuật tán nhuyễn') !== false))
        {
            ...
            if ($catbe == 1)
            {
                ...
                $ngaykq_pt_catbe = strtotime($ngaykq_pt_ttt) + 900;
                $ngaykq_pt_catbe = date('YmdHi', $ngaykq_pt_catbe);
                $phoihop = 1;
            }
            if ($catdichkinh == 1)
    ...
    if ($ma_loai_kcb == 3 or $ma_loai_kcb == 9)
    {
        ...
        foreach ($chitiets as $chitiet)
        {
           ...
           //////// ĐỔI GIỜ RỬA LỆ ĐẠO ///////
            ...
            elseif (((strpos($chitiet, 'TTT ') !== false) or (strpos($chitiet, 'Thủy tinh thể nhân tạo') !== false) or (strpos($chitiet, 'Thủy tinh thể đơn tiêu cự kéo dài') !== false) or (strpos($chitiet, 'Thủy tinh thể') !== false)) and $check_pt_tannhuyen == 1)
            {
                $chitiet = str_replace('<NGAY_KQ>' . $ngaykq[0] . '</NGAY_KQ>', '<NGAY_KQ>' . $ngaykq_pt_ttt . '</NGAY_KQ>', $chitiet);
                $content3mod = $content3mod . '<CHI_TIET_DVKT>' . $chitiet . '</CHI_TIET_DVKT>';
            }
            ...
            elseif (strpos($chitiet, 'Mở bao sau bằng phẫu thuật') !== false)
            {
			 ...
			 //////// ĐỔI GIỜ PHẪU THUẬT IOL THÌ 2 + CẮT DỊCH KÍNH ///////
            elseif (strpos($chitiet, 'Phẫu thuật cố định IOL thì hai + cắt dịch kính') !== false)
            {
                $chitiet = str_replace('<NGAY_KQ>' . $ngaykq[0] . '</NGAY_KQ>', '<NGAY_KQ>' . $ngaykq_pt_iolthi2_catdichkinh . '</NGAY_KQ>', $chitiet);
                if ($phoihop == 1)
                {
                    $chitiet = str_replace('<NGAY_YL>' . $ngayyl[0] . '</NGAY_YL>', '<NGAY_YL>' . $ngaykq_pt_iolthi2_catdichkinh . '</NGAY_YL>', $chitiet);
                }
                $content3mod = $content3mod . '<CHI_TIET_DVKT>' . $chitiet . '</CHI_TIET_DVKT>';
            }
...
<div class="alert alert-secondary" role="alert">
 ...
 <a target="_blank" rel="noopener noreferrer" href="check-mstbt.php?id=2" >[MSTBT] Kiểm tra bản Convert MỚI</a><br>
</div>
'
?>
